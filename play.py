...
from admins import admins as a
...
from gets import get_file_name
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"<b>🔴 WAITING ERROR 🔴 \nUser {user.first_name} COULDN'T JOIN YOUR GROUP! MAKE SURE USER IS NOT BANNED 😉 IN THIS GROUP."
                        "\n\nOr MANUALLY ADD ASSISTANT TO YOUR GROUP AND RETRY</b>",
                    )
    try:
...
async def deezer(client: Client, message_: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("YOURPLAYLIST", callback_data="YOURPLAYLIST"),
                InlineKeyboardButton("MENU ⏯ ", callback_data="MENU"),
            ],
            [InlineKeyboardButton(text="Listen On Deezer 🎬", url=f"{url}")],
            [InlineKeyboardButton(text="CANCEL", callback_data="cls")],
        ]
    )
...
async def jiosaavn(client: Client, message_: Message):
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"<b>🔴 WAITING ERROR 🔴 \nUser {user.first_name} COULDN'T JOIN THE  GROUP! MAKE SURE USER IS NOT BANNED 😉 IN THIS GROUP."
                        "\n\nOr MANUALLY ADD @intimacymusic AND RETRY</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
    ...
    try:    
        ...
        if duuration > DURATION_LIMIT:
            await cb.message.edit(f"MUSIC YOU REQUESTED IS LONGER THAN {DURATION_LIMIT}MIN ARE NOT ALLOWED")
            return
    except:
        pass    
    keyboard = InlineKeyboardMarkup(
...
async def lol_cb(b, cb):
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await b.send_photo(chat_id,
            photo="final.png",
            caption=f"#⃣  THE SONG {r_by.mention} REQUESTED IS <b>queued</b> AT POSITION {position}!",
            reply_markup=keyboard,
        )
        os.remove("final.png")
        ...
        await b.send_photo(chat_id,
            photo="final.png",
            reply_markup=keyboard,
            caption=f"▶️ <b>PLAYING</b> CURRENTLY IS THE SONG WHICH  {r_by.mention} REQUESTED VIA YOUTUBE 🔥",
        )
